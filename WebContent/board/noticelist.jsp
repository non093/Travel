...
<%	
	...
	boolean isAdmin = "관리자".equals(admin);
...
<%
	...
	//검색
	List<BoardReplyVO> noticeBoardList;
	if(isSearch){ 
...
<script>
	$(function(){
		...
		$(".listSizeHref").click(function(e){
			...
			if(isSearch){
				href="noticelist.jsp?type=<%=type%>&key=<%=key%>&size="+target;
			}
			else{
				href="noticelist.jsp?size="+target;
			}
			location.href=href;
		});
...
<div class="outbox" style="width:1000px">
	...
	<div class="row margin">
		...
		<tbody class="listBlock">
			...
			<%}else if(noticeBoardList.isEmpty() && noticeList.isEmpty()){ %>
					<tr><td colspan="6">게시물이 존재하지 않습니다.</td></tr>
			<%}else{
					for(BoardReplyVO noticeDto : noticeBoardList){ %>
					<tr>
						<td width="10%" class="head-color"><%=noticeDto.getBoard_head() %></td>
						<td width="50%" class="left">
							<a href="detail.jsp?board_no=<%=noticeDto.getBoard_no() %>#cl"><%=noticeDto.getBoard_title() %></a>
							[<a href="detail.jsp?board_no=<%=noticeDto.getBoard_no() %>#rl"><%=noticeDto.getReplynum() %></a>]
						</td>
	...
	<div class="row floatbox">
		...
		<div class="center">
			<ul class="pagination">
					<%if(isSearch){ %>
						<li><a href="noticelist.jsp?type=<%=type %>&key=<%=key %>&p=<%=startBlock-1%>" class="previous nav">&lt;이전</a></li>
					<%}else { %>
						<li><a href="noticelist.jsp?p=<%=startBlock-1%>" class="previous nav">&lt;이전</a></li>
					<%} %>
				  	...
				  	<%for(int i=startBlock; i<=endBlock; i++) {%>
				  		...
				  		<%if(isSearch){%>
							<a href="noticelist.jsp?type=<%=type %>&key=<%=key %>&p=<%=i%>"><%=i %></a>
				  		<%}else{ %>
							<a href="noticelist.jsp?p=<%=i%>"><%=i %></a>
				  		<%} %>
						</li>
				  	<%} %>
				 	
				 	<%if(isSearch){ %>
						<li><a href="noticelist.jsp?type=<%=type %>&key=<%=key %>&p=<%=endBlock+1%>" class="next nav">다음&gt;</a></li>
				 	<%}else{ %>
						<li><a href="noticelist.jsp?p=<%=endBlock+1%>" class="next nav">다음&gt;</a></li>
				 	<%} %>
			</ul>
		</div>
	</div>
	...
	<form action="noticelist.jsp" method="post">
		...
		<%if(isSearch){ %>
			<a href="noticelist.jsp" class="reset">초기화&#10226;</a>
		<%} %>
	</form>
	</div>
