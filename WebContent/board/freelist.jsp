...
<script>
	$(function(){
		...
		$(".headHref").click(function(e){
			...
			if(isSearch){
				href="freelist.jsp?type=<%=type%>&key=<%=key%>&head="+target;
			}
			else{
				href="freelist.jsp?head="+target;
			}
			location.href=href;
		});
		...
		$(".listSizeHref").click(function(e){
			...
			if(isSearch){
				href="freelist.jsp?type=<%=type%>&key=<%=key%>&head=<%=board_head%>&size="+target;
			}
			else{
				href="freelist.jsp?head=<%=board_head%>&size="+target;
			}
			location.href=href;
		});
...
<div class="outbox" style="width:1000px">
	...
	<div class="row margin">
		...
		<tbody class="listBlock">
			...
			%>
				<%
					for(BoardReplyVO freeDto : freeList){
				%>
				<tr>
					<td class="head-color"><%=freeDto.getBoard_head() %></td>
					<td width="50%" class="left">
						<a href="detail.jsp?board_no=<%=freeDto.getBoard_no() %>#cl"><%=freeDto.getBoard_title() %></a>
						[<a href="detail.jsp?board_no=<%=freeDto.getBoard_no() %>#rl"><%=freeDto.getReplynum() %></a>]
					</td>
	...
	<div class="row floatbox">
		...
		<div class="center">
			<ul class="pagination">
					<%if(isSearch){ %>
						<li><a href="freelist.jsp?type=<%=type %>&key=<%=key %>&p=<%=startBlock-1%>&head=<%=board_head %>" class="previous nav">&lt;이전</a></li>
					<%}else { %>
						<li><a href="freelist.jsp?p=<%=startBlock-1%>&head=<%=board_head %>" class="previous nav">&lt;이전</a></li>
					<%} %>
				  	...
				  	<%for(int i=startBlock; i<=endBlock; i++) {%>
				  		...
				  		<%if(isSearch){%>
							<a href="freelist.jsp?type=<%=type %>&key=<%=key %>&p=<%=i%>&head=<%=board_head%>"><%=i %></a>
				  		<%}else{ %>
							<a href="freelist.jsp?p=<%=i%>&head=<%=board_head%>"><%=i %></a>
				  		<%} %>
						</li>
				  	<%} %>
				 	
				 	<%if(isSearch){ %>
						<li><a href="freelist.jsp?type=<%=type %>&key=<%=key %>&p=<%=endBlock+1%>&head=<%=board_head %>" class="next nav">다음&gt;</a></li>
				 	<%}else{ %>
						<li><a href="freelist.jsp?p=<%=endBlock+1%>&head=<%=board_head %>" class="next nav">다음&gt;</a></li>
				 	<%} %>
			</ul>
		</div>
	</div>
	...
	<form action="freelist.jsp" method="post">
		...
		<%if(isSearch){ %>
			<a href="freelist.jsp" class="reset">초기화&#10226;</a>
		<%} %>
	</form>
	</div>
