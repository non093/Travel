...
<script>
	$(function(){
		...
		$(".headHref").click(function(e){
			...
			if(isSearch){
				href="entirelist.jsp?type=<%=type%>&key=<%=key%>&head="+target;
			}
			else{
				href="entirelist.jsp?head="+target;
			}
			location.href=href;
		});
		...
		$(".listSizeHref").click(function(e){
			...
			if(isSearch){
				href="entirelist.jsp?type=<%=type%>&key=<%=key%>&head=<%=board_head%>&size="+target;
			}
			else{
				href="entirelist.jsp?head=<%=board_head%>&size="+target;
			}
			location.href=href;
		});
...
<div class="outbox" style="width:1000px">
	...
	<div class="row margin">
		...
		<tbody class="listBlock">
			...
			%>
				<%
					for(BoardReplyVO entireDto : entireList){
				%>
				<tr>
					<td width="10%" class="head-color"><%=entireDto.getBoard_head() %></td>
					<td width="50%" class="left">
						<a href="detail.jsp?board_no=<%=entireDto.getBoard_no() %>#cl"><%=entireDto.getBoard_title() %></a>
						[<a href="detail.jsp?board_no=<%=entireDto.getBoard_no() %>#rl"><%=entireDto.getReplynum() %></a>]
					</td>
	...
	<div class="row floatbox">
		...
		<div class="center">
			<ul class="pagination">
					<%if(isSearch){ %>
						<li><a href="entirelist.jsp?type=<%=type %>&key=<%=key %>&p=<%=startBlock-1%>&head=<%=board_head %>" class="previous nav">&lt;이전</a></li>
					<%}else { %>
						<li><a href="entirelist.jsp?p=<%=startBlock-1%>&head=<%=board_head %>" class="previous nav">&lt;이전</a></li>
					<%} %>
				  	...
				  	<%for(int i=startBlock; i<=endBlock; i++) {%>
				  		...
				  		<%if(isSearch){%>
							<a href="entirelist.jsp?type=<%=type %>&key=<%=key %>&p=<%=i%>&head=<%=board_head%>"><%=i %></a>
				  		<%}else{ %>
							<a href="entirelist.jsp?p=<%=i%>&head=<%=board_head%>"><%=i %></a>
				  		<%} %>
						</li>
				  	<%} %>
				 	
				 	<%if(isSearch){ %>
						<li><a href="entirelist.jsp?type=<%=type %>&key=<%=key %>&p=<%=endBlock+1%>&head=<%=board_head %>" class="next nav">다음&gt;</a></li>
				 	<%}else{ %>
						<li><a href="entirelist.jsp?p=<%=endBlock+1%>&head=<%=board_head %>" class="next nav">다음&gt;</a></li>
				 	<%} %>
			</ul>
		</div>
	</div>
	...
	<form action="entirelist.jsp" method="post">
		...
		<%if(isSearch){ %>
			<a href="entirelist.jsp" class="reset">초기화&#10226;</a>
		<%} %>
	</form>
	</div>
